...
import { CreateClientDto, ILead, UpdateClientDto } from '@/shared/interfaces/interfaces'
...
import { createNewClient, updateClient } from '@/redux/client/clientSlice'
...
const Drawer = (props: IProps) => {
    ...
    const handleInputChange = (e: any) => {
        ...
        if (name === 'phoneNumber' || name === 'buyerLoanOfficerPhone') {
    ...
    const submitNewClient = () => {
        ...
        if (!validation.IsEmptyString(values.firstName) && !validation.IsEmptyString(values.lastName) && !errorPhoneNumber && !errorEmail) {
            ...
            if (props.data) {
                let updateData: UpdateClientDto = {
                    data: values,
                ...
                dispatch(updateClient(updateData)).then((res) => {
                    try {
                ...
                dispatch(createNewClient(data)).then((res) => {
                    try {
                        notify(res.payload.success, res.payload.message)
    ...
    const onSelectAddressAutoComplete  = (value: any) => {
        setValues({
            ...values,
            address: value.streetLine+' '+value.city+', '+value.state+' '+value.zipcode+' '+value.secondary,
        })
    }
                                            ...
                                            <div className="px-4 sm:px-6  w-full bg-white">
                                                <div className="flex items-start justify-between">
                                                    <Dialog.Title className="">
                                                        <Typography variant="h2" color="primary" className='my-0'>{props.data ? 'Edit a Client' : 'Add a New Client'}</Typography>
                                                    </Dialog.Title>
