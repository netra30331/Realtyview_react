...
interface IProps {
  sortType: string
  sortField: string
  sortFieldOptions:Array<any>
  filterData: Function
  changeSortType: Function
  changeSortField: Function
}
const SortAdvanced = (props:IProps) => {
  const label: string = props.sortFieldOptions.find(x => x.value === props.sortField)?.label;
  return (
    <>
              ...
              <div className="px-0 py-0 w-full ">
                <div className='py-3 px-5 flexitems-center cursor-pointer w-full'>
                  ...
                  <Select
                    ...
                    value={{value: props.sortField, label: label ?? props.sortField}}
                    onChange={(value)=>props.changeSortField(value.value)}
                  />
                </div>
                <div className='px-5'>
                ...
                <div className='w-full flex justify-center py-[15px] pb-7'>
                  <Button className='w-[146px] text-15' onClick={()=>props.filterData()}>Sort</Button>
                </div>
              </div>
            </Popover.Panel>
          </Transition>
